...
document.addEventListener('DOMContentLoaded', () => {
    bouton_clavier = document.querySelectorAll('.ligne_clavier');
    eventListenerSetup();
})
...
function eventListenerSetup(){
    ...
    bouton_clavier.forEach(btn => {
        btn.addEventListener("mouseup", function (event){

            ligne = "mot_" + num_ligne;

            
            list_lettre = document.getElementById(ligne).children;
            console.log(event);
            let action = event.path[0].attributes[1].nodeValue;
            
            console.log(action);

            if (action.includes('Key') && num_lettre < 5 && continuer) {

                lettre_tapper = event.target.textContent;

                ajout_lettre(lettre_tapper);

            } else if(action.includes('Semicolon') && num_lettre < 5 && continuer){

                lettre_tappez = event.key;
        
                ajout_lettre(lettre_tapper);
        
            } else if(action.includes('Backspace') && num_lettre > 0 && continuer){

                effacer_lettre()

            } else if(action.includes('Enter') && num_lettre == 5 && continuer){
...
function ajout_lettre (lettre_tapper){
    ...
    console.log(list_lettre[num_lettre]);
